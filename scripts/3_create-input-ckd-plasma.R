...
## Open datasets
pmet <- readRDS("data/plasma_metabolites.RDS") # plasma metabolites
df <- readRDS('data/helius_ckd_xgb.RDS') # IDs and CKD yes/no
pmet <- pmet[which(rownames(pmet) %in% df$ID),] # filter out DKD

df <- df[match(rownames(pmet), df$ID), ] # put IDs of CKD dataframe in same order as metdf
rownames(pmet)
df$ID
