...
plot_features_tests_reg <- function(input_path, output_path, top_n=10, outcome_name='Outcome', x_lab='Levels', residuals=FALSE){
    ...
    yfile <- ifelse(residuals==TRUE, 'y_reg.txt', 'y_unscaled.txt')
...
plot_feature_importance_microbiome <- function(path_true, top_n){
    theme_Publication <- function(base_size=12, base_family="sans") {
        library(grid)
        library(ggthemes)
        (theme_foundation(base_size=base_size, base_family=base_family)
...
plot_features_tests_class <- function(input_path, output_path, top_n=10, labels=c("1", "0")){
    ...
    for(j in 1:(ncol(dd)-1)){
        ...
        pl <- ggplot(df, aes(x=y, y=Feature, fill=y))+
            ...
            #geom_dotplot(binaxis = "y", stackdir="center", color="black", alpha=0.5, dotsize = 0.7)+
            scale_fill_manual(values = cl)+
            #ggbeeswarm::geom_beeswarm(color="black", cex=2.0, alpha=0.5)+
...
plot_features_tests_top <- function(input_path, output_path, top_n=20, nrow=4, labels){
    ...
    pl <- ggplot(df, aes(x=y, y=values))+
        geom_violin(aes(fill=y), alpha = 0.7, trim = TRUE)+
        scale_fill_manual(values = cl) +
        geom_boxplot(width=0.1, fill="white")+
        theme_Publication()+
        theme(legend.position = 'none')+
        ...
        ggpubr::stat_compare_means(comparisons = comps, paired = F)+
        facet_wrap(~ features, nrow=nrow, scales = 'free')
    ggsave(pl, path = plot_path, filename = paste0('top_',top_n,'_features_boxplots.pdf'), device = 'pdf', width = 22, height = 16)
    ggsave(pl, path = plot_path, filename = paste0('top_',top_n,'_features_boxplots.svg'), device = 'svg', width=15, height = 15)
}
...
plot_feature_importance_class_urine <- function(path_true, top_n){
    ...
    r <- rio::import(file.path(path_true, 'feature_importance.txt'))
    r <- r %>% arrange(-RelFeatImp)
    a <- rio::import('data/Info_urine_metabolites.xlsx')
